...
export const updateUserRole = async (id: string, role: string, token: string) => {
  const response = await axios.patch(`http://localhost:5000/api/admin/users/${id}/role`, { role }, {
    headers: { Authorization: `Bearer ${token}` },
  });
  return response.data;
};
...
export const restoreProduct = async (id: string, token: string) => {
  ...
  const response = await axios.patch(`http://localhost:5000/api/products/${id}/restore`, {}, {
    headers: { Authorization: `Bearer ${token}` },
  });
  return response.data;
};
